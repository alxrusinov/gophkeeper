...
func TestHttpHandler_GetCredentialsList(t *testing.T) {
	...
	successSource := []model.Credentials{{
	...
	testUsecase.On("GetCredentialsList", mock.Anything, mock.Anything).Return(successSource, nil)
	...
	app.Get("/api/credentials", testHandler.GetCredentialsList)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			switch tt.name {
			case tests[0].name:
				server.GET("/api/credentials").Expect().Status(tt.resCode)
			case tests[1].name:
				testUsecase.On("GetCredentialsList", mock.Anything, mock.Anything).Unset()
				testUsecase.On("GetCredentialsList", mock.Anything, mock.Anything).Return(tt.source, &customerrors.NotFound{})
				server.GET("/api/credentials").Expect().Status(tt.resCode)
			case tests[2].name:
			...
			case tests[3].name:
				...
				testAuth.On("GetUserFromContext", mock.Anything).Return(successUser, nil)
				testUsecase.On("GetCredentialsList", mock.Anything, mock.Anything).Unset()
				testUsecase.On("GetCredentialsList", mock.Anything, mock.Anything).Return(tt.source, errors.New("error"))
				server.GET("/api/credentials").Expect().Status(tt.resCode)
