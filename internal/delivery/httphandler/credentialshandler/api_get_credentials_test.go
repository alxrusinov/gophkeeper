...
func TestHttpHandler_GetCredentials(t *testing.T) {
	testUsecase := usecasemock.NewUsecaseMock()
	testAuth := authMock.NewAuthMock()
	testHandler := NewCredentialsHandler(testUsecase, testAuth)
	...
	app.Get("/api/credentials/{id}", testHandler.GetCredentials)
	...
	errHandler := NewCredentialsHandler(testUsecase, errAuth)

	errApp := iris.New()

	errApp.Get("/api/credentials/{id}", errHandler.GetCredentials)
	...
	testUsecase.On("GetCredentials", mock.Anything, mock.Anything, successNoteID).Return(&model.Credentials{
		ID:     successNoteID,
	...
	testUsecase.On("GetCredentials", mock.Anything, mock.Anything, errID).Return(new(model.Credentials), errors.New("err"))
	testUsecase.On("GetCredentials", mock.Anything, mock.Anything, noSource).Return(new(model.Credentials), &customerrors.NotFound{Err: errors.New("err")})
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {

			switch tt.name {
			case tests[0].name:
				server.GET(fmt.Sprintf("/api/credentials/%s", tt.arg)).Expect().Status(tt.resCode)
			case tests[1].name:
				server.GET(fmt.Sprintf("/api/credentials/%s", tt.arg)).Expect().Status(tt.resCode)
			case tests[2].name:
				...
				errServer.GET(fmt.Sprintf("/api/credentials/%s", tt.arg)).Expect().Status(tt.resCode)
			case tests[3].name:
				testAuth.On("GetUserFromContext", mock.Anything).Unset()
				testAuth.On("GetUserFromContext", mock.Anything).Return(successUser, nil)
				server.GET(fmt.Sprintf("/api/credentials/%s", tt.arg)).Expect().Status(tt.resCode)
