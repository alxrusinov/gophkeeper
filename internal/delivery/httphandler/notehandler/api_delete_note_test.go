...
func TestHttpHandler_DeleteNote(t *testing.T) {
	testUsecase := usecasemock.NewUsecaseMock()
	testAuth := authMock.NewAuthMock()
	testHandler := NewNoteHandlerHandler(testUsecase, testAuth)
	...
	testUsecase.On("DeleteNote", mock.Anything, mock.Anything).Return(successSource, nil)
